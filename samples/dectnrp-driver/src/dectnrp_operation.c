...
K_MEM_SLAB_DEFINE_STATIC(operations_slab, sizeof(struct dectnrp_driver_op), 10,
                         4);

/**
 * @brief Allocate a new operation and initialize basic fields common to all
 * operations.
 *
 * @param ctx
 * @return struct dectnrp_driver_op*
 */
static struct dectnrp_driver_op *
alloc_operation(enum dectnrp_driver_op_type type, struct net_if *iface,
                uint16_t channel) {
...
int dectnrp_alloc_rssi1_operation(struct dectnrp_driver_op **new_op,
  ...
  struct dectnrp_driver_op *op =
      alloc_operation(DECTNRP_DRIVER_OP_RSSI1, iface, channel);
  if (op) {
